...
if(isset($_GET['tid']) AND $_GET['tid'] > 0)
{
	...
	$ticketQrySQL = sprintf("SELECT creator, firstname, lastname, subject, ts_created, ts_updated, tick.status, IFNULL(server,0) server, serv.name srvname 
								...
								WHERE ticketid = %d LIMIT 1", DBPREFIX, DBPREFIX, DBPREFIX, $tid);
...
$rows = query_fetch_assoc( "SELECT * FROM `".DBPREFIX."client` WHERE `clientid` = '".$ticketdata['creator']."' LIMIT 1" );
...
<div class="well">
	...
	<table class="table table-striped table-bordered table-condensed">
		...
		<tr>
			<td>Client</td>
			<td><?php echo (htmlspecialchars($ticketdata['firstname'], ENT_QUOTES).' '.htmlspecialchars($ticketdata['lastname'],ENT_QUOTES));?></td>
		</tr>
		<tr>
			<td>Subject</td>
		...
		<tr>
			<td>Server</td>
			<td><?php echo htmlspecialchars($ticketdata['srvname'], ENT_QUOTES); ?></td>
		</tr>
	</tbody></table>
